class Variant:
    ...
    def classify_quality(self, use_sam=True):
        ...
        # AB 조건: heterozygous 또는 homozygous-alt에서만 체크
        ab_condition_high = (
            ...
            (self.sample1_gt == (0, 0) and self.sample2_gt == (0, 0))  # 둘 다 hom-ref면 AB 무시
        )
        ab_condition_low = (
        ...
        if (self.sample1_dp and self.sample1_dp >= 20 and self.sample2_dp and self.sample2_dp >= 20 and
            ...
            if not use_sam or (self.sample1_validated and self.sample2_validated and
                               self.sample1_read_count >= 5 and self.sample2_read_count >= 5):
                return "high_quality"
        ...
        if (self.sample1_dp and self.sample1_dp >= 10 and self.sample2_dp and self.sample2_dp >= 10 and
            ...
            ab_condition_low):
            if not use_sam or (self.sample1_read_count >= 1 and self.sample2_read_count >= 1):
                return "low_quality"

        return "incorrect"

    def validate_with_sam(self, sample_id, reads, expected_base):
        """샘플별 SAM 읽기로 변이 검증."""
        read_count = 0
        base_count = 0
        for read in reads:
        ...
        if sample_id == "sample1":
            ...
            self.sample1_validated = base_count > 0
        else:
            self.sample2_read_count = read_count
            self.sample2_validated = base_count > 0
        #print(f"Validated {self.chrom}:{self.position} for {sample_id} - Expected: {expected_base}, Reads: {read_count}, Matches: {base_count}")
    ...
    def __str__(self):
        return (f"{self.chrom}:{self.position} {self.ref}>{self.alt} "
                ...
                f"Sample1_Reads={self.sample1_read_count} Sample2_Reads={self.sample2_read_count}")